...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.pollCandidate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.poll-candidates.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('experience_points') ? 'is-invalid' : '' }}" type="number" name="experience_points" id="experience_points" value="{{ old('experience_points', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\PollCandidate::COMPANY_SIZE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('company_size') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="company_size_{{ $key }}" name="company_size" value="{{ $key }}" {{ old('company_size', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('candidate_program') ? 'is-invalid' : '' }}" name="candidate_program_id" id="candidate_program_id">
                    @foreach($candidate_programs as $id => $entry)
                        <option value="{{ $id }}" {{ old('candidate_program_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_initial') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_initial" id="is_initial" value="1" {{ old('is_initial', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_final') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_final" id="is_final" value="1" {{ old('is_final', 0) == 1 ? 'checked' : '' }}>
