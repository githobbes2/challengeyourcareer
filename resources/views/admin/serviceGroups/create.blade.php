...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.serviceGroup.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.service-groups.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_items') ? 'is-invalid' : '' }}" name="service_items[]" id="service_items" multiple>
                    @foreach($service_items as $id => $service_item)
                        <option value="{{ $id }}" {{ in_array($id, old('service_items', [])) ? 'selected' : '' }}>{{ $service_item }}</option>
                    @endforeach
                </select>
