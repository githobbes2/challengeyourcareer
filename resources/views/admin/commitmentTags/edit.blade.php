...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.commitmentTag.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.commitment-tags.update", [$commitmentTag->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.commitmentTag.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $commitmentTag->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\CommitmentTag::COLOR_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('color') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="color_{{ $key }}" name="color" value="{{ $key }}" {{ old('color', $commitmentTag->color) === (string) $key ? 'checked' : '' }}>
