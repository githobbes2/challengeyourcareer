...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.programType.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.program-types.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('outplacement') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="outplacement" id="outplacement" value="1" {{ old('outplacement', 0) == 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_groups') ? 'is-invalid' : '' }}" name="service_groups[]" id="service_groups" multiple>
                    @foreach($service_groups as $id => $service_group)
                        <option value="{{ $id }}" {{ in_array($id, old('service_groups', [])) ? 'selected' : '' }}>{{ $service_group }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('session_types') ? 'is-invalid' : '' }}" name="session_types[]" id="session_types" multiple>
                    @foreach($session_types as $id => $session_type)
                        <option value="{{ $id }}" {{ in_array($id, old('session_types', [])) ? 'selected' : '' }}>{{ $session_type }}</option>
                    @endforeach
                </select>
