    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.events.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="number" name="duration" id="duration" value="{{ old('duration', '60') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('program_types') ? 'is-invalid' : '' }}" name="program_types[]" id="program_types" multiple>
                    @foreach($program_types as $id => $program_type)
                        <option value="{{ $id }}" {{ in_array($id, old('program_types', [])) ? 'selected' : '' }}>{{ $program_type }}</option>
                    @endforeach
                </select>
