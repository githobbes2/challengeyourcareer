...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.jobOfferCandidate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.job-offer-candidates.update", [$jobOfferCandidate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('job_offer') ? 'is-invalid' : '' }}" name="job_offer_id" id="job_offer_id" required>
                    @foreach($job_offers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('job_offer_id') ? old('job_offer_id') : $jobOfferCandidate->job_offer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('candidate') ? 'is-invalid' : '' }}" name="candidate_id" id="candidate_id">
                    @foreach($candidates as $id => $entry)
                        <option value="{{ $id }}" {{ (old('candidate_id') ? old('candidate_id') : $jobOfferCandidate->candidate->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\JobOfferCandidate::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $jobOfferCandidate->status) === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_favorite') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_favorite" id="is_favorite" value="1" {{ $jobOfferCandidate->is_favorite || old('is_favorite', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('curriculum') ? 'is-invalid' : '' }}" name="curriculum_id" id="curriculum_id">
                    @foreach($curricula as $id => $entry)
                        <option value="{{ $id }}" {{ (old('curriculum_id') ? old('curriculum_id') : $jobOfferCandidate->curriculum->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('request_mediation') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="request_mediation" id="request_mediation" value="1" {{ $jobOfferCandidate->request_mediation || old('request_mediation', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('mediation_status') ? 'is-invalid' : '' }}" name="mediation_status" id="mediation_status">
                    ...
                    @foreach(App\Models\JobOfferCandidate::MEDIATION_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('mediation_status', $jobOfferCandidate->mediation_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('mediation_notes') ? 'is-invalid' : '' }}" name="mediation_notes" id="mediation_notes">{{ old('mediation_notes', $jobOfferCandidate->mediation_notes) }}</textarea>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('mediation_private_notes') ? 'is-invalid' : '' }}" name="mediation_private_notes" id="mediation_private_notes">{{ old('mediation_private_notes', $jobOfferCandidate->mediation_private_notes) }}</textarea>
