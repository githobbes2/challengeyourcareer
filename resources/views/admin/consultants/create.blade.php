    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.consultants.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('consultant_type') ? 'is-invalid' : '' }}" name="consultant_type_id" id="consultant_type_id" required>
                    @foreach($consultant_types as $id => $entry)
                        <option value="{{ $id }}" {{ old('consultant_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('session_skills') ? 'is-invalid' : '' }}" name="session_skills" id="session_skills">{{ old('session_skills') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('skill_tags') ? 'is-invalid' : '' }}" name="skill_tags[]" id="skill_tags" multiple>
                    @foreach($skill_tags as $id => $skill_tag)
                        <option value="{{ $id }}" {{ in_array($id, old('skill_tags', [])) ? 'selected' : '' }}>{{ $skill_tag }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('url_linkedin') ? 'is-invalid' : '' }}" type="text" name="url_linkedin" id="url_linkedin" value="{{ old('url_linkedin', '') }}">
