    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.service-types.update", [$serviceType->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('code') ? 'is-invalid' : '' }}" type="text" name="code" id="code" value="{{ old('code', $serviceType->code) }}" required>
                @if($errors->has('code'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $serviceType->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\ServiceType::TYPE_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('type') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="type_{{ $key }}" name="type" value="{{ $key }}" {{ old('type', $serviceType->type) === (string) $key ? 'checked' : '' }} required>
