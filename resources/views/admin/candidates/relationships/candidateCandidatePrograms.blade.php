<div class="m-3">
    @can('candidate_program_create')
        <div style="margin-bottom: 10px;" class="row">
            <div class="col-lg-12">
                <a class="btn btn-primary" href="{{ route('admin.candidate-programs.create') }}">
                    {{ trans('global.add') }} {{ trans('cruds.candidateProgram.title_singular') }}
                </a>
            </div>
        </div>
    @endcan
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-candidateCandidatePrograms">
                    <thead>
                        <tr>
                            <th width="10">
                    ...
                    <tbody>
                        @foreach($candidatePrograms as $key => $candidateProgram)
                            <tr data-entry-id="{{ $candidateProgram->id }}">
                                ...
                                <td>
                                    {{ $candidateProgram->functional_area->name ?? '' }}
                                </td>
                                <td>
                                    {{ $candidateProgram->profesional_level->name ?? '' }}
                                </td>
                                <td>
                                    {{ $candidateProgram->industry_sector->name ?? '' }}
                                </td>
                                <td width="150">
...
@endcan
  ...
  let table = $('.datatable-candidateCandidatePrograms:not(.ajaxTable)').DataTable({ buttons: dtButtons })
