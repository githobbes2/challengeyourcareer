    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.session-templates.update", [$sessionTemplate->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $sessionTemplate->description) }}</textarea>
            ...
            <div class="row border-bottom mb-3">
                    ...
                    <div class="form-group">
                        <select class="form-control select2 {{ $errors->has('session_type_' . $i) ? 'is-invalid' : '' }}" name="session_type_{{ $i }}" id="session_type_{{ $i }}">
                            @foreach($session_types as $id => $session_type)
                                <option value="{{ $id }}" {{ (old('session_type_' . $i) ? old('session_type_' . $i) : $item->id ?? '') == $id ? 'selected' : '' }}>{{ $session_type }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-3">
                    <div class="form-group">
                        <input class="form-control {{ $errors->has('session_quantity_' . $i) ? 'is-invalid' : '' }}" type="number" name="session_quantity_{{ $i }}" id="session_quantity_{{ $i }}" value="{{ old('session_quantity_' . $i, $item->pivot->quantity) }}" step="1" min="1" max="20" required>
                ...
                <div class="col-3">
                    <div class="form-group">
                        <input class="form-control {{ $errors->has('session_duration_' . $i) ? 'is-invalid' : '' }}" type="number" name="session_duration_{{ $i }}" id="session_duration_{{ $i }}" value="{{ old('session_duration_' . $i, $item->pivot->duration) }}" step="1" min="0" max="6000" required>
            ...
            @for ($i=(1+count($sessionTemplate->session_types)); $i <= 12; $i++)
            <div class="row border-bottom mb-3">
                <div class="col-6">
                    <div class="form-group">
                        <select class="form-control select2 {{ $errors->has('session_type_' . $i) ? 'is-invalid' : '' }}" name="session_type_{{ $i }}" id="session_type_{{ $i }}">
