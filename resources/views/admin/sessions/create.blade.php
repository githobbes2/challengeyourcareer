...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sessions.store") }}" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('session_type') ? 'is-invalid' : '' }}" name="session_type_id" id="session_type_id" required>
                            @foreach($session_types as $id => $entry)
                                <option value="{{ $id }}" {{ old('session_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                            @foreach($users as $id => $entry)
                                <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : Auth::user()->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('user'))
                    ...
                    <div class="row">
                        ...
                        <div class="col-4">
                            <div class="form-group">
                                ...
                                <input class="form-control {{ $errors->has('duration') ? 'is-invalid' : '' }}" type="number" name="duration" id="duration" value="{{ old('duration', '60') }}" step="1" required>
                    ...
                    <div class="form-group">
                        ...
                        @foreach(App\Models\Session::STATUS_RADIO as $key => $label)
                            <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                                <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', 'p') === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                <label for="candidates">Participantes Adicionales</label>
                <select class="form-control select2" name="candidates[]" id="candidates" multiple>
                    @foreach($candidates as $candidate)
                        <option value="{{ $candidate->id }}">{{ $candidate->full_name }}</option>
                    @endforeach
                </select>
                <span class="help-block">Seleccione candidatos adicionales a los participantes del programa seleccionado.</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('private_notes') ? 'is-invalid' : '' }}" name="private_notes" id="private_notes">{{ old('private_notes') }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('manager_score') ? 'is-invalid' : '' }}" type="number" name="manager_score" id="manager_score" value="{{ old('manager_score', '') }}" step="1" min="1" max="5">
