...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.candidateService.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.candidate-services.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_item') ? 'is-invalid' : '' }}" name="service_item_id" id="service_item_id" required>
                    @foreach($service_items as $id => $entry)
                        <option value="{{ $id }}" {{ old('service_item_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date_service') ? 'is-invalid' : '' }}" type="text" name="date_service" id="date_service" value="{{ old('date_service') }}">
