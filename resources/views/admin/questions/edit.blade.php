            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('development_area') ? 'is-invalid' : '' }}" name="development_area_id" id="development_area_id">
                    @foreach($development_areas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('development_area_id') ? old('development_area_id') : $question->development_area->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('points_' . $i) ? 'is-invalid' : '' }}" type="number" name="points_{{ $i }}" id="points_{{ $i }}" value="{{ old('points_' . $i, $question['points_' . $i]) }}" step="0.01" min="0" max="1">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('session_types_' . $i) ? 'is-invalid' : '' }}" name="session_types_{{ $i }}[]" id="session_types_{{ $i }}" multiple>
                            @foreach($session_types as $id => $session_type)
                                <option value="{{ $id }}" {{ (in_array($id, old('session_types_' . $i, [])) || (array_key_exists($i, $score_session_types) && in_array((int)$id, $score_session_types[$i]))) ? 'selected' : '' }}>{{ $session_type }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('experience_points_' . $i) ? 'is-invalid' : '' }}" type="number" name="experience_points_{{ $i }}" id="experience_points_{{ $i }}" value="{{ old('experience_points_' . $i, $question['experience_points_' . $i]) }}" step="1" min="0" max="100">
