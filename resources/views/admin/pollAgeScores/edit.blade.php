...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pollAgeScore.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.poll-age-scores.update", [$pollAgeScore->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.pollAgeScore.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $pollAgeScore->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('order') ? 'is-invalid' : '' }}" type="number" name="order" id="order" value="{{ old('order', $pollAgeScore->order) }}" step="1">
                @if($errors->has('order'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('age_start') ? 'is-invalid' : '' }}" type="number" name="age_start" id="age_start" value="{{ old('age_start', $pollAgeScore->age_start) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('end_range') ? 'is-invalid' : '' }}" type="number" name="end_range" id="end_range" value="{{ old('end_range', $pollAgeScore->end_range) }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('professional_levels') ? 'is-invalid' : '' }}" name="professional_levels[]" id="professional_levels" multiple>
                    @foreach($professional_levels as $id => $professional_level)
                        <option value="{{ $id }}" {{ (in_array($id, old('professional_levels', [])) || $pollAgeScore->professional_levels->contains($id)) ? 'selected' : '' }}>{{ $professional_level }}</option>
                    @endforeach
                </select>
