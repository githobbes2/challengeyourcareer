...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.candidateProgram.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.candidate-programs.update", [$candidateProgram->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        <label>{{ trans('cruds.candidateProgram.fields.candidate') }}</label>
                        <select class="form-control select2 {{ $errors->has('candidate') ? 'is-invalid' : '' }}" name="hold_candidate_id" id="hold_candidate_id" disabled>
                            @foreach($candidates as $id => $entry)
                                <option value="{{ $id }}" {{ (old('candidate_id') ? old('candidate_id') : $candidateProgram->candidate->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                        @if($errors->has('candidate'))
                            <span class="text-danger">{{ $errors->first('candidate') }}</span>
                        @endif
                        <span class="help-block">{{ trans('cruds.candidateProgram.fields.candidate_helper') }}</span>
                    </div>
                    <div class="form-group">
                        <label for="program_id">{{ trans('cruds.candidateProgram.fields.program') }}</label>
                        <select class="form-control select2 {{ $errors->has('program') ? 'is-invalid' : '' }}" name="hold_program_id" id="hold_program_id" disabled>
                            @foreach($programs as $id => $entry)
                                <option value="{{ $id }}" {{ (old('program_id') ? old('program_id') : $candidateProgram->program->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        @foreach(App\Models\CandidateProgram::STATUS_RADIO as $key => $label)
                            <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                                <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $candidateProgram->status) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control date {{ $errors->has('program_start_date') ? 'is-invalid' : '' }}" type="text" name="program_start_date" id="program_start_date" value="{{ old('program_start_date', $candidateProgram->program_start_date) }}">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control date {{ $errors->has('program_end_date') ? 'is-invalid' : '' }}" type="text" name="program_end_date" id="program_end_date" value="{{ old('program_end_date', $candidateProgram->program_end_date) }}">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input class="form-control date {{ $errors->has('relocation_date') ? 'is-invalid' : '' }}" type="text" name="relocation_date" id="relocation_date" value="{{ old('relocation_date', $candidateProgram->relocation_date) }}">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="relocation_company_id">{{ trans('cruds.candidateProgram.fields.relocation_company') }}</label>
                                <input class="form-control {{ $errors->has('relocation_company') ? 'is-invalid' : '' }}" type="text" name="relocation_company" id="relocation_company" value="{{ old('relocation_company', $candidateProgram->relocation_company) }}">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('industry_sector') ? 'is-invalid' : '' }}" name="industry_sector_id" id="industry_sector_id">
                                    @foreach($industry_sectors as $id => $entry)
                                        <option value="{{ $id }}" {{ (old('industry_sector_id') ? old('industry_sector_id') : $candidateProgram->industry_sector->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('functional_area') ? 'is-invalid' : '' }}" name="functional_area_id" id="functional_area_id">
                                    @foreach($functional_areas as $id => $entry)
                                        <option value="{{ $id }}" {{ (old('functional_area_id') ? old('functional_area_id') : $candidateProgram->functional_area->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control select2 {{ $errors->has('profesional_level') ? 'is-invalid' : '' }}" name="profesional_level_id" id="profesional_level_id">
                                    @foreach($profesional_levels as $id => $entry)
                                        <option value="{{ $id }}" {{ (old('profesional_level_id') ? old('profesional_level_id') : $candidateProgram->profesional_level->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                    @endforeach
                                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('internal_notes') ? 'is-invalid' : '' }}" name="internal_notes" id="internal_notes">{{ old('internal_notes', $candidateProgram->internal_notes) }}</textarea>
