...
class MessengerController extends Controller
{
    public function index()
    {
        ...
        //meseenger for CANDIDATE users
        if(Auth::user()->is_candidate && !Auth::user()->is_admin) {
            $users = User::all()
                ->except(Auth::id())
                ->sortBy('name');

            return view('candidate.messages', compact('users', 'topics', 'title', 'unreads'));
        }
    ...
    public function showMessages(QaTopic $topic)
    {
        ...
        return view('admin.messenger.show', compact('topic', 'newMessageID', 'unreads'));
    }
    ...
    public function showReply(QaTopic $topic)
    {
        ...
        $receiver = $receiverOrCreator->name;

        return view('admin.messenger.reply', compact('topic', 'receiver', 'unreads'));
    }
